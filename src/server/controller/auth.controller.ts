...
const verifySchema = z.object({ publicKey: publicKeySchema, signedNonce: z.string().min(1) });
...
export async function me(_req: NextRequest, ctx: { publicKey?: string }) {
  ...
  return ok({ publicKey: ctx.publicKey });
}

/** Dev/e2e only: mint a session for a public key without a signature. */
export async function testLogin(req: NextRequest) {
  if (env.ENABLE_TEST_LOGIN !== 'true') {
    throw new AppError('NOT_FOUND', 'Not found', 404);
  }
  const { publicKey } = challengeSchema.parse(await req.json());
  const { sessionId } = await authService.mintSession(publicKey);
  const res = ok({ ok: true, publicKey });
  setSessionCookie(res, sessionId);
  return res;
}

...
export const authService = {
  async createChallenge(
    ...
    await authNonceRepo.insert({ nonce, publicKey, expiresAt });
  ...
  async verifyAndCreateSession(
    ...
    const op = tx.operations.find((o) => o.type === 'manageData') as
      ...
      | undefined;
    if (!op || op.name !== AUTH_KEY || !op.value) {
      throw new AppError('UNAUTHORIZED', 'Challenge operation not found in transaction', 401);
    }
    const nonce = op.value.toString('utf8');

    const matched = await authNonceRepo.findActive(publicKey, nonce, new Date());
    if (!matched) {
      throw new AppError('UNAUTHORIZED', 'Challenge not found or expired', 401);
    }

    await authNonceRepo.consume(matched.nonce);
    return authService.mintSession(publicKey);
  },

  async mintSession(publicKey: string): Promise<{ sessionId: string }> {
    const expiresAt = new Date(Date.now() + env.SESSION_TTL_SECONDS * 1000);
    const sessionId = await sessionRepo.insert({ publicKey, expiresAt });
    return { sessionId };
  },
